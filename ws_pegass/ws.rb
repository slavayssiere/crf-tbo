...
class PegassApp < Sinatra::Base
  ...
  general_info(
    {
      version: '0.0.1',
      title: 'Pegass Webservices',
      description: 'Pegass for admin'
    }
  )
  ...
  helpers do
    def get_connexion
      ...
      logger.info "HTTP_USERNAME present ? #{request.env['HTTP_USERNAME']}"
      logger.info "HTTP_PASSWORD present ? #{request.env['HTTP_PASSWORD']}"
